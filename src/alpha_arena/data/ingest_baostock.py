...
def stock_daily(
    ...
    storage_format: str = typer.Option(
        "csv", "--storage-format", "-s", help="Storage format (csv or parquet)"
    ),
...
def get_adjustment_dates(start_year: str, end_year: str):
    ...
    for year in range(int(start_year) - 1, int(end_year) + 1):
        for month in [6, 12]:
            # 获取当月的第三个周一
            third_monday = nth_weekday_of_month(
                year, month, weekday=2, n=3
            )  # weekday=0表示周一
...
def csi300_stocks_by_date_range(
    helper: BaostockHelper,
    start_year: str = typer.Argument(
        ..., help="Start year (YYYY) for csi300 stock list"
    ),
    ...
    for i in range(len(adjust_dates)):
        ...
        end_date = (
            adjust_dates[i + 1] - pd.Timedelta(days=1)
            if i + 1 < len(adjust_dates)
            else pd.to_datetime(f"{end_year}-12-31")
        )
        stocks = get_csi300_stocks(helper, start_date)
        if "sh.600004" in stocks:
            logger.info(
                "Sample stock is in csi300",
...
def csi300_stocks(
    ...
    if file_path is None:
        ...
        file_path = (
            RAW_DATA_DIR / f"csi300_stocks_{start_year}_{end_year}.{storage_format}"
        )
    ...
    with BaostockHelper() as helper:
        ...
        for stock in all_stocks[:2]:
            ...
            df["in_csi300"] = df["date"].apply(
                lambda x: check_stock_in_csi300(stock_date_ranges, stock, x)
            )
            all_fds.append(df)
