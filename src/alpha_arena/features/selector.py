...
def select_lstm_feature_columns(
    ...
    """
    resolved_exclude_keywords = exclude_keywords or _EXCLUDE_KEYWORDS
    resolved_feature_prefixes = feature_prefixes or _FEATURE_PREFIXES
    cols = []
    for c in df.columns:
        if c in _RAW_COLS:
            continue
        if any(k in c for k in resolved_exclude_keywords):
            continue
        if c in target_columns:
            continue
        if (
            resolved_feature_prefixes
            and any(c.startswith(p) for p in resolved_feature_prefixes)
        ) or not resolved_feature_prefixes:
            cols.append(c)
